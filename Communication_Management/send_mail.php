...
if ($fetch_data != 0) {
    foreach ($fetch_data as $data) {
        ...
        $sendmailStatus = send_mail('support@bitotechnologies.com', $sendMessage, $mailSubject);
        $id = $data['id'];
        $update_date_time = date('d-m-Y h:i:s');
        $send_date = date('d-m-Y');
        $send_time = date('h:i:s');
        $update_compliane_send_alert = ['sended_message' => $sendMessage, 'mail_subject' => $mailSubject, 'user_mail' => $email, 'send_status' => $sendmailStatus, 'server_response' => $sendmailStatus, 'send_mail_date' => $send_date, 'send_mail_time' => $send_time, 'send_message_type' => 'automatic',   'update_by' => 'automatic', 'update_date_time' => $update_date_time];
